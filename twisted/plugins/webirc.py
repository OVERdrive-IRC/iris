...
class QWebIRCServiceMaker(object):
    ...
    def makeService(self, config):
        ...
        if config['https']:
            ...
            i = internet.SSLServer(int(config['https']), site, ssl_factory(config['privkey'], config[
                                   'certificate'], certificateChainFile=config["certificate-chain"]), interface=config['ip'])
        else:
            i = internet.TCPServer(
                int(config['port']), site, interface=config['ip'])
