...
class QWebIRCClient(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        if command == "CAP":
            ...
            if "ACK" in params:
                if "sasl" in params[-1].split(" "):
                    if (self.authUser and self.authSecret):
                        self.write(
                            "AUTHENTICATE " + config.atheme["sasl_type"])
                        self.saslauth = True
        ...
        if (command == "AUTHENTICATE"):
            ...
            authtext = base64.b64encode(
                '\0'.join([self.authUser, self.authUser, self.authSecret]))
    ...
    def stopSasl(self):
        ...
        if (self.saslauth):
            ...
            self.handleCommand(
                "904", "QWebIRC", ["*", "SASL authentication timed out"])
    ...
    def connectionMade(self):
        ...
        nick, ident, ip, realname, hostname, pass_ = f["nick"], f[
            "ident"], f["ip"], f["realname"], f["hostname"], f.get("password")
        ...
        if config.irc["webirc_mode"] == "webirc":
            self.write("WEBIRC %s qwebirc %s %s" %
                       (config.irc["webirc_password"], hostname, ip))
            ...
            self.write("USER %s bleh bleh :%s - %s" %
                       (ident, dispip, realname))
        else:
    ...
    def connectionLost(self, reason):
        if self.lastError:
            self.disconnect(
                "Connection to IRC server lost: %s" % self.lastError)
        else:
...
class QWebIRCFactory(protocol.ClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        self.publisher.event(
            ["disconnect", "Connection to IRC server failed."])
...
def createIRC(*args, **kwargs):
    ...
    if config.irc["ssl"]:
        ...
        reactor.connectSSL(
            server, port, f, ClientContextFactory(), bindAddress=bindToAddress)
    else:
        reactor.connectTCP(server, port, f, bindAddress=bindToAddress)
    return f

if __name__ == "__main__":
    e = createIRC(lambda x: 2, nick="slug__moo", ident="mooslug",
                  ip="1.2.3.6", realname="mooooo", hostname="1.2.3.4")
    reactor.run()
