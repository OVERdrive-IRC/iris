...
class AthemeEngine(resource.Resource):
    ...
    def logout(self, request):
        ...
        try:
            result = {"success": True, "output": None}
            result["output"] = self.do_xmlrpc(
                self.conn.atheme.logout, (token[0], user[0]))
            if result["output"] is not None:
                response = json.dumps(result)
            else:
                response = json.dumps(None)
    ...
    def command(self, request):
        ...
        try:
            result = {"success": True, "output": None}
            result["output"] = self.do_xmlrpc(self.conn.atheme.command, (token[0], user[
                                              0], "0.0.0.0", service[0], command[0]) + tuple(params))
            if result["output"] is not None:
                response = json.dumps(result)
            else:
                response = json.dumps(None)
