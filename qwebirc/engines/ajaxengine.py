...
class IRCSession:
    ...
    def subscribe(self, channel, notifier):
        timeout_entry = reactor.callLater(
            config.tuneback["http_ajax_request_timeout"], self.timeout, channel)
    ...
    def flush(self, scheduled=False):
        ...
        if t < self.throttle:
            if not self.schedule:
                self.schedule = reactor.callLater(
                    self.throttle - t, self.flush, True)
            return
        else:
...
class AJAXEngine(resource.Resource):
    ...
    def newConnection(self, request):
        ...
        def proceed(hostname):
            kwargs = dict(nick=nick, ident=ident, ip=ip,
        ...
        if config.irc["webirc_mode"] == "":
            ...
            def errback(failure):
                ...
                proceed(ip)
            qdns.lookupAndVerifyPTR(
                ip, timeout=[config.tuneback["dns_timeout"]]).addCallbacks(callback, errback)
    ...
    def subscribe(self, request):
        ...
        self.getSession(request).subscribe(
            SingleUseChannel(request), request.notifyFinish())
        return NOT_DONE_YET
