...
class FeedbackEngine(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        # basic checksum to stop really lame kiddies spamming, see feedback.js
        # for js version
        checksum = 0
        ...
        if checksum != sentchecksum:
            raise FeedbackException(
                "Bad checksum: %d vs. %d" % (sentchecksum, checksum))
        ...
        factory = factorytype(fromEmail=config.feedbackengine[
                              "from"], toEmail=config.feedbackengine["to"], file=email, deferred=defer.Deferred())
        reactor.connectTCP(
            config.feedbackengine["smtp_host"], config.feedbackengine["smtp_port"], factory)
        self.__hit()
        return "1"
