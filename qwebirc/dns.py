        ...
        def gotAResult(a_records):
            if ip in a_records:
                ...
                raise VerificationException(
                    "IP mismatch: %s != %s%s" % (repr(ip), repr(ptr), repr(a_records)))
        lookupAs(
            ptr, *args, **kwargs).addCallback(gotAResult).addErrback(d.errback)

    lookupPTR(
        ip, *args, **kwargs).addCallback(gotPTRResult).addErrback(d.errback)
    return d
