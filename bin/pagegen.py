...
def jslist(name, debug):
    ui = pages.UIs[name]
    if debug:
        x = [pages.JS_BASE, ui.get("extra", []), pages.DEBUG, [
...
def _getgitid():
    try:
        p = subprocess.Popen(
            ["git", "show", "--pretty=oneline", "--quiet"], stdout=subprocess.PIPE)
...
def producehtml(name, debug):
    ...
    csshtml = "\n".join("  <link rel=\"stylesheet\" href=\"%s%s\" type=\"text/css\"/>" %
                        (config.frontend["static_base_url"], x) for x in css)
    jshtml = "\n".join("  <script type=\"text/javascript\" src=\"%s%s\"></script>" %
                       (config.frontend["static_base_url"], x) for x in js)

    div = ui.get("div", "")
    customcss = "\n".join("  <link rel=\"stylesheet\" href=\"%s%s\" type=\"text/css\"/>" %
                          (config.frontend["static_base_url"], x) for x in ui.get("customcss", []))
    customjs = "\n".join("  <script type=\"text/javascript\" src=\"%s%s\"></script>" %
                         (config.frontend["static_base_url"], x) for x in ui.get("customjs", []))
