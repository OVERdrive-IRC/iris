...
def jarit(src):
    try:
        p = subprocess.Popen(
            ["java", "-jar", "bin/yuicompressor-2.3.5.jar", src], stdout=subprocess.PIPE)
...
def jmerge_files(prefix, suffix, output, files, *args, **kwargs):
    ...
    try:
        ...
        if not JAVA_WARNING_SURPRESSED:
            ...
            print("warning: minify: %s (not minifying -- javascript will be HUGE)." %
                  e, file=sys.stderr)
        try:
            f = open(o, "rb")
            compiled = f.read()
...
def main(outputdir=".", produce_debug=True):
    ...
    for (uiname, value) in pages.UIs.items():
        ...
        shutil.copy2(os.path.join(outputdir, "static", "css", uiname + "-" +
                                  ID + ".css"), os.path.join(outputdir, "static", "css", uiname + ".css"))

        mcssname = os.path.join("css", uiname + ".mcss")
        if os.path.exists(mcssname):
            mcssdest = os.path.join(
                outputdir, "static", "css", uiname + ".mcss")
            shutil.copy2(mcssname, mcssdest)
            shutil.copy2(mcssdest, os.path.join(
                outputdir, "static", "css", uiname + "-" + ID + ".mcss"))
